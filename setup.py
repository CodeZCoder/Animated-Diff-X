...
def download_default_models():
    ...
    try:
        ...
        try:
            # Try to load the model from our application's models directory
            sd_dir = os.path.join(models_dir, "stable_diffusion")
            ...
            logger.info("Default Stable Diffusion model not found, downloading to application models folder...")
            # This will download and cache the model to our application's models directory
            sd_dir = os.path.join(models_dir, "stable_diffusion")
            StableDiffusionPipeline.from_pretrained(
                "runwayml/stable-diffusion-v1-5", 
                torch_dtype=torch.float32,
