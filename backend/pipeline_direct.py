...
class DirectFrameGenerator:
    """
    A direct frame generator that bypasses diffusers library constraints
    to ensure exact number of frames and steps are used.
    """
    ...
    def generate_frames(
        ...
        """
        Generate frames with direct control over the process
        ...
        # Generate each frame with explicit control
        for frame_idx in range(num_frames):
            ...
            if init_image and self.img2img_pipeline:
                ...
                # Force the exact number of steps by directly controlling the process
                result = self._generate_img2img_frame(
                    prompt=prompt,
                ...
                # Text-to-video mode
                result = self._generate_txt2img_frame(
                    prompt=prompt,
                    negative_prompt=negative_prompt,
                    height=height,
                    width=width,
    ...
    def _generate_txt2img_frame(
        ...
        """
        Generate a single frame using text-to-image with direct control over steps
        """
        ...
        # Denoising loop with explicit step control
    ...
    def _generate_img2img_frame(
        ...
        """
        Generate a single frame using image-to-image with direct control over steps
        """
        # Get the text embeddings
...
class AnimateDiffPipeline:
    ...
    def generate(
        ...
        try:
            ...
            # Create the direct frame generator
            frame_generator = DirectFrameGenerator(sd_pipeline, img2img_pipeline)
            frame_generator.set_callback(self.callback)
            
            # Generate the video frames
            logger.info(f"Generating {num_frames} frames with {num_inference_steps} steps")
            
            # Generate all frames with direct control
